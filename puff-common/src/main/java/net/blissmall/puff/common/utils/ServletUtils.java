...
public class ServletUtils {
    ...
    // 静态文件后缀
//	private final views String[] staticFiles = StringUtils.split(GlobalHelper.getConfig("webcontext.staticFile"), ",");
    private final static String[] staticFiles = null;

    // 动态映射URL后缀
//	private final views String urlSuffix = GlobalHelper.getUrlSuffix();
    ...
    public static boolean isStaticFile(String uri) {
        if (staticFiles == null) {
            try {
                throw new Exception("检测到“app.properties”中没有配置“webcontext.staticFile”属性。配置示例：\n#静态文件后缀\n" + "webcontext.staticFile=.css,.js,.png,.jpg,.gif,.jpeg,.bmp,.ico,.swf,.psd,.htc,.crx,.xpi,.exe,.ipa,.apk");
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        // if ((StringUtils.startsWith(uri, "/views/") ||
